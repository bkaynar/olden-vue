<template>
    <div class="q-pa-md rounded-borders">
        ...
        <div class="row items-center justify-between q-mb-lg">
            ...
            <div v-if="!isMobile" class="row q-gutter-sm">
                <q-btn v-for="c in categories" :key="c.key" rounded :flat="activeTab !== c.key"
                    :color="activeTab === c.key ? 'primary' : 'grey-7'"
                    :text-color="activeTab === c.key ? 'white' : 'grey-4'" :label="c.label" @click="activeTab = c.key"
                    class="modern-tab-btn" :class="{ 'active-tab': activeTab === c.key }" />
            </div>
            <!-- Mobilde boş div -->
            <div v-else></div>

            <!-- Tümünü Gör butonunu sadece desktop'ta göster -->
            <q-btn v-if="!isMobile" flat rounded dense class="view-all-btn" label="Tümünü Gör"
        ...
        <div v-if="!isMobile" class="row q-gutter-lg justify-start">
            <template v-if="filteredCasino.length > 0">
                <div v-for="(game, idx) in filteredCasino.slice(0, 6)" :key="idx"
                ...
                <div class="swiper-container">
                    <swiper :modules="modules" :slides-per-view="'auto'" :centered-slides="true" :space-between="20"
                        :initial-slide="0" :loop="true" :navigation="{
                            nextEl: `#${uniqueCasinoSwiperId}-next`,
                            prevEl: `#${uniqueCasinoSwiperId}-prev`,
                        }" class="Casino-main-swiper" @swiper="onSwiper" @slide-change="onSlideChange">
                        <swiper-slide v-for="(game, idx) in filteredCasino.slice(0, 6)" :key="idx" class="game-slide">
                            <div class="modern-game-card-mobile relative flex items-center justify-center overflow-hidden"
                                ...
                                <div v-if="activeSlideIndex === idx"
                                    class="game-info absolute bottom-0 left-0 right-0 p-2 show">
                                    <div class="text-white text-weight-medium text-caption">{{ game.name }}</div>
                                    <div class="text-grey-4 text-caption">{{ game.provider }}</div>
                                </div>
                            </div>
                    ...
                    <!-- Casino Navigation Buttons -->
                    <div :id="`${uniqueCasinoSwiperId}-prev`" class="Casino-nav-prev"></div>
                    <div :id="`${uniqueCasinoSwiperId}-next`" class="Casino-nav-next"></div>
                </div>
            </template>
            <template v-else>
...
function generateUniqueCasinoId() {
    return 'Casino-swiper-' + Math.random().toString(36).substr(2, 9)
}
...
const props = defineProps<{ casinoOyunlari?: any[] }>()
...
const uniqueCasinoSwiperId = ref(generateUniqueCasinoId())
...
onMounted(() => {
    ...
    console.log('Casino props:', props.casinoOyunlari)
})
...
// casinoOyunlari prop'u varsa ve içinde gorsel/url varsa, fallbackCasino yerine onu kullan
const casinoList = computed(() => {
    if (Array.isArray(props.casinoOyunlari) && props.casinoOyunlari.length > 0 && props.casinoOyunlari.some(item => item.gorsel)) {
        return props.casinoOyunlari.map(item => ({
            name: item.gorsel ? (item.gorsel.split('/').pop()?.split('.')[0] || 'Casino Oyun') : 'Casino Oyun',
            provider: '',
            category: 'Casino',
            cover: item.gorsel,
            url: item.url,
    ...
    return fallbackCasino
})

// Dinamik kategoriler
const categories = computed(() => {
    const list = casinoList.value
...
/* Casino Navigation Buttons - TAMAMEN AYRI CLASS'LAR */
.Casino-nav-prev,
.Casino-nav-next {
    position: absolute;
    top: 50%;
...
.Casino-nav-prev:hover,
.Casino-nav-next:hover {
    background: rgba(255, 255, 255, 0.25);
    transform: scale(1.05);
    border-color: rgba(102, 126, 234, 0.4);
}

.Casino-nav-prev:hover::after {
    border-color: transparent #fff transparent transparent;
}

.Casino-nav-next:hover::after {
